...
with sess_1.as_default():
    with tf_graph_1.as_default():
        ...
        model_1 = "supervised/models/convothello.ckpt"
        if os.path.isfile(model_1 + ".meta"):
            saver_1.restore(sess_1, model_1)
            print("loaded graph 1 as " + model_1)
            
pool_path = '/storage/hlynur/models/reinforcement_drill_convothello/'
for batch in range(batches):
    print("new batch prints")
    print(batch)
    if batch > 0:
        log_message = reinforcement_log(model_1, model_2, wins_1, wins_2, games_per_batch, "logs_rl.txt", batch)
        rl_reinforce_1.updateModel()
        save_path = saver_1.save(sess_1, pool_path + "drill_" + str(batch) + ".ckpt")
    with sess_2.as_default():
        with tf_graph_2.as_default():
            tf.global_variables_initializer().run()
            saver_2 = tf.train.Saver(tf.global_variables())
            #model_2 = choose_opponent(batch)
            model_2 = choose_opponent_nr(batch, pool_path)
            #next_nr = random.randint(0, batch)
            if os.path.isfile(model_2 + ".meta"):
                saver_2.restore(sess_2, model_2) 
                print("loaded graph 2 as " + model_2)
            else:
                print("not finding" + model_2)
    print(batch)
    graph_1, graph_2 = (-1, 1)
    wins_1, wins_2 = (0, 0)
    fails = 0
    for n in range(games_per_batch):
        ...
        choices = random.sample(set(range(60)), 16)
        while True:
