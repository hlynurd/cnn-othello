...
def prediction_accuracy(data_path, len_games=64):
    lengths = []
    successes = []
    validation_matches = get_all_matches(data_path)
...
sess.run(init_op)
current_model = "models/layers7filters64.ckpt.meta"
saver = tf.train.Saver()
#saver = tf.train.import_meta_graph(current_model + ".meta")
...
for i in range(prev_stop, prev_stop+iterations):
    ...
    if (i % 20 is 0) and (i > 0 and i < 101) or (i+1) == (iterations+prev_stop) or (i % 250 is 0):  
