...
def policy_net_7_layers(filters):
    img_data = tf.placeholder(tf.float32, shape=[None, 8, 8, 28], name="img_data")
    keep_prob = tf.placeholder(tf.float32, name="keep_prob")
    ...
    conv7_out = conv_nonparams(conv6_out, conv7_weights, conv7_biases, keep_prob)
    
    #scoring layer
    k = 1; i = o; o = 2;
    score_weights = weight_variable([k, k, i, o])
    score_biases = bias_variable([o])
    score_out = conv_nonparams(conv7_out, score_weights, score_biases, keep_prob)
...
def policy_net_8_layers(filters):
    img_data = tf.placeholder(tf.float32, shape=[None, 8, 8, 28], name="img_data")
    keep_prob = tf.placeholder(tf.float32, name="keep_prob")
    ...
    conv8_out = conv_nonparams(conv7_out, conv8_weights, conv8_biases, keep_prob)
    
    #scoring layer
    k = 1; i = o; o = 2;
    score_weights = weight_variable([k, k, i, o])
    score_biases = bias_variable([o])
    score_out = conv_nonparams(conv8_out, score_weights, score_biases, keep_prob)
