...
class PolicyGradientREINFORCEothello(object):
  ...
  def create_variables(self):
    with tf.name_scope("model_inputs"):
      ...
      self.states = tf.placeholder(tf.float32, (None, self.state_dim, self.state_dim, 1), name="states")
    ...
    with tf.name_scope("predict_actions"):
      ...
      if os.path.isfile(self.current_model):
        saver.restore(self.session, self.current_model)
  ...
  def updateModel(self):
    ...
    saver = tf.train.Saver()
    saver.save(self.session, self.current_model)
    # clean up
    self.cleanUp()
