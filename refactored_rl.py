...
for batch in range(batches):
    if batch > 0:
        startc = time.time()
        log_message = reinforcement_log(model_1, model_2, wins_1, wins_2, games_per_batch, "rl_logs.txt")
        rl_reinforce_1.updateModel()
        endc = time.time()
        sumc = sumc + (endc - startc)
        #print("C: " + str(endc-startc))
    graph_1, graph_2 = (-1, 1)
    wins_1, wins_2 = (0, 0)
    fails = 0
