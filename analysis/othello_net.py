...
def create_no_graph():
    ...
    return img_data, train_step, optimizer, labels, loss, predictions, learn_rate, score_out
    
def create_example_graph():
    img_data = tf.placeholder(tf.float32, shape=[None, 8, 8, 1], name="img_data")
    #convolutional layers 
    i = 1; o = 2;
    conv1_weights = conv_weights(i, o)
    conv1_biases = conv_biases(o)
    ...
    #final layer
    predictions = softmax(conv1_out, o)
    ...
    loss = semantic_loss(conv1_out, labels, o)    
    optimizer = tf.train.AdamOptimizer(learn_rate)
    train_step = optimizer.minimize(loss)
    
    return img_data, train_step, optimizer, labels, loss, predictions, learn_rate, conv1_out, conv1_weights
