...
def avg_error(data_path, sess, len_games = 32):
    errors = []
...
def prediction_accuracy(data_path, len_games=32):
    successes = []
    validation_matches = get_all_matches(data_path)
    #validation_matches = validation_matches[0:len_games]
...
sess.run(init_op)
current_model = "models/convothello.ckpt"
...
for ite in range(prev_stop, prev_stop+iterations):
    ...
    if i is 0:
        ...
        log_experiment("logfile_sl.txt", log_time, log_acc, log_loss, i)
